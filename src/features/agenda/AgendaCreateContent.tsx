/**
 * LV-09.1B.6.3B.1 — Corpo funcional do fluxo de criação da Agenda.
 *
 * Este componente concentra TODA a lógica de criação de prazo/compromisso:
 * estado, permissões, carregamento paginado de assignments, submit,
 * single-flight, detecção de rascunho e confirmação de descarte.
 *
 * NÃO contém shell de diálogo. Pode ser montado por:
 *  - `AgendaCreateDialog` (wrapper fino em superfície `dialog`);
 *  - a rota canônica `/app/agenda/novo` (superfície `page`).
 *
 * Comunicação com o consumidor:
 *  - `onCreated` — item recém-criado;
 *  - `onRequestClose` — solicitação de fechamento/saída do fluxo.
 *
 * Fechamento externo (dialog): o consumidor chama `handle.requestClose()`
 * via ref. A regra é a mesma do botão Cancelar (single-flight, rascunho,
...
export type AgendaCreateSurface = "page" | "dialog";

export interface AgendaCreateContentProps {
  ...
  readonly surface: AgendaCreateSurface;
}

export interface AgendaCreateContentHandle {
  requestClose(): void;
}

// ---- Componente principal -------------------------------------------------

export const AgendaCreateContent = React.forwardRef<
  AgendaCreateContentHandle,
  AgendaCreateContentProps
>(function AgendaCreateContent(props, ref) {
  const {
    active,
    environment,
    context,
  ...
  // Ao ativar, restaura estado (mesma semântica de "abrir" o antigo diálogo).
  React.useEffect(() => {
    if (!active) return;
  ...
  }, [active, currentCaseId, environment, context]);
  ...
  }, [active, currentCaseId, environment, context, assignmentsAttempt]);
  ...
  const requestClose = React.useCallback(() => {
    ...
    onRequestClose();
  }, [deadlineForm, appointmentForm, onRequestClose]);
  ...
  }, [deadlineForm, environment, context, onCreated, onRequestClose, shouldCloseAfterCreate]);
  ...
  }, [appointmentForm, environment, context, onCreated, onRequestClose, shouldCloseAfterCreate]);
  ...
  // Layout muda ligeiramente entre superfícies: no diálogo há altura máxima
  // e overflow; na página o conteúdo flui normalmente.
...
// Também expõe uma prop `isDraft` calculada externamente? Não — o rascunho é
// interno ao Content por decisão desta etapa (LV-09.1B.6.3B.1).
export { ASSIGNMENTS_MAX_PAGES, ASSIGNMENTS_PAGE_LIMIT };
